...
// Attempt to send email alerts asynchronously/quietly even if DB query had issues or table wasn't found
$mailSent = false;
try {
    if (!empty($email)) {
        $mailer = new Mailer();
...
if ($executed || $mailSent) {
    ...
    http_response_code(200); // Return 200 with success: false so fetch().json() does not throw Unexpected end of JSON input
    echo json_encode([
        'success' => false,
        'message' => 'Failed to process submission. ' . ($dbError ?? ($conn->error ?? 'Please check connection.'))
    ]);
}
exit();
?>
