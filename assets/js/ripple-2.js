+(function ($) {
  ...
  var Ripples = function (el, options) {
    ...
    $(window).on("resize", function () {
      if (
        that.$el.outerWidth() != that.canvas.width ||
        that.$el.outerHeight() != that.canvas.height
      ) {
  ...
  Ripples.prototype = {
    ...
    computeTextureBoundaries: function () {
      ...
      if (backgroundSize == "cover") {
        var scale = Math.max(
          winWidth / this.backgroundWidth,
          winHeight / this.backgroundHeight,
        );
      ...
      if (backgroundX == "left") backgroundX = winOffset.left;
        ...
        backgroundX =
          winOffset.left +
          ((winWidth - backgroundWidth) * parseFloat(backgroundX)) / 100;
      } else {
      ...
      if (backgroundY == "top") backgroundY = winOffset.top;
        ...
        backgroundY =
          winOffset.top +
          ((winHeight - backgroundHeight) * parseFloat(backgroundY)) / 100;
      } else {
  ...
  $.fn.ripples = function (option) {
    return this.each(function () {
      if (!supportsWebGL)
        throw new Error(
          "Your browser does not support at least one of the following: WebGL, OES_texture_float extension, OES_texture_float_linear extension.",
        );
