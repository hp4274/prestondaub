...
(function ($) {
  ...
  ////////////////////////////////////////////////////
  // 01. PreLoader Js (Disabled - page transitions removed as per client request)
  /*
  ...
  var sideMenuList = $(
    ".tp-offcanvas-menu nav > ul > li button.tp-menu-close, .tp-offcanvas-menu nav > ul li.has-dropdown > a, .tp-offcanvas-menu nav > ul li.has-dropdown > ul > li.menu-item-has-children > a",
  );
  ...
  // 14. Close Handlers //
  $(".cartmini-close-btn, .body-overlay, .tp-offcanvas-2-close-btn").on(
    "click",
    function () {
      $(".tp-search-area, .cartmini__area, .tp-offcanvas-2-area").removeClass(
  ...
  $(".tp-service-vp-wrap .tp-service-vp-item")
    .on("mouseenter", function () {
      $(this)
        .addClass("active")
        .parent()
        .siblings()
        .find(".tp-service-vp-item")
        .removeClass("active");
    })
  ...
  if (portfolioLists.length > 0) {
    portfolioLists.forEach((portfolio) => {
      gsap.set(portfolio, {
        opacity: 0.7,
        transform:
          "perspective(4000px) translate3d(0px, 0px, 0px) rotateX(90deg) scale(0.5, 0.5)",
      });
  ...
  // Improved: swipe / drag support for each service background slider (pointer, touch and mouse fallback) + autoplay
  (function () {
    const sections = Array.from(
      document.querySelectorAll(".tp-hero-cst-bottom"),
    );
    if (sections.length === 0) return;

    sections.forEach((section) => {
      const sliderWrapper = section.querySelector(".tp-hero-cst-bottom-slider");
      const bgContainer = section.querySelector("#service-bg-img");
      const btnItems = Array.from(
        section.querySelectorAll(
          ".tp-hero-cst-bottom-btn ul li.service__item-8",
        ),
      );
      if (!sliderWrapper || !bgContainer || btnItems.length === 0) return;

      let current = btnItems.findIndex((i) => i.classList.contains("active"));
      if (current === -1) current = 0;

      btnItems.forEach((el, idx) => {
      ...
      function normalizeIndex(i) {
        if (i < 0) return btnItems.length - 1;
        if (i >= btnItems.length) return 0;
        return i;
      }
      ...
      function goTo(index) {
        ...
        const rel = btnItems[index].getAttribute("rel");

        // Immediate swap without GSAP animations
        $(bgContainer).removeClass().addClass(rel);
        btnItems[current].classList.remove("active");
      ...
      function shouldIgnoreEvent(e) {
        if (e.target.closest("a, button, input, textarea, select, label"))
          return true;
        if (e.target.closest(".tp-hero-cst-bottom-btn")) return true;
        return false;
      }
      ...
      function onMove(e) {
        ...
        const dx = x - startX;
        if (bgContainer) gsap.to(bgContainer, { x: dx * 0.12, duration: 0 });
      }
      ...
      function onUp(e) {
        ...
        isDown = false;
        if (bgContainer)
          gsap.to(bgContainer, { x: 0, duration: 0.3, ease: "power2.out" });
        const x = getClientX(e);
        const dx = x - startX;
        if (Math.abs(dx) > threshold) {
          ...
          // minor tap/click - no swipe: leave
        }
        resetAutoplay();
      }

      // Attach listeners to slider wrapper and bg container
      [sliderWrapper, bgContainer].forEach((el) => {
      ...
      document.addEventListener("mousedown", function (e) {
        if ([sliderWrapper, bgContainer].some((t) => t.contains(e.target)))
          onDown(e);
      });
      ...
      // keyboard navigation
      bgContainer.setAttribute("tabindex", "0");
      [sliderWrapper, bgContainer].forEach(
        (t) => t.setAttribute && t.setAttribute("tabindex", "0"),
      );
      [sliderWrapper, bgContainer].forEach((t) =>
  ...
  if (fadeArray_items.length > 0) {
    ...
    t.forEach((t, a) => {
      ...
      ("top" == e && (g.y = -i),
        ...
        "bottom" == e && (g.y = i),
        "right" == e && (g.x = i),
        1 == r && (g.scrollTrigger = { trigger: t, start: "top 85%" }),
        gsap.from(t, g));
    });
  }
      ...
      gsap.to(sections, {
        ...
        scrollTrigger: {
          ...
          scrub: 1,
            ...
            "+=" +
            document.querySelector(".tp-skill-pb-panel-wrap").offsetWidth,
        },
      });
    }
  });
  ...
  ///////////////////////////////////////////////////
  // 41. fade-class-active
  if (typeof gsap !== "undefined" && $(".tp_fade_anim").length > 0) {
      ...
      gsap.to(sections, {
        ...
        scrollTrigger: {
          ...
          scrub: 1,
            ...
            "+=" + document.querySelector(".tp-funfact-panel-wrap").offsetWidth,
        },
      });
    }
  });
  ...
  if ($(".tp-awards-vp-move-thumbs-wrapper").length > 0) {
    if (!isMobile()) {
      const moveThumbsWrapper = document.querySelector(
        ".tp-awards-vp-move-thumbs-wrapper",
      );
      const startThumbsCaption = document.querySelector(
        ".tp-awards-vp-start-thumbs-caption",
      );
      const moveThumbsParent = document.querySelectorAll(
        ".tp-awards-vp-start-thumbs-wrapper .tp-awards-vp-start-move-thumb",
      );
      const moveThumbs = document.querySelectorAll(
        ".tp-awards-vp-start-thumbs-wrapper .tp-awards-vp-move-thumb-inner",
      );
      const overlappingThumbs = document.querySelectorAll(
        ".tp-awards-vp-end-thumbs-wrapper .tp-awards-vp-end-move-thumb",
      );
        ...
        if (startThumbsCaption) {
          gsap.to(startThumbsCaption, {
            scrollTrigger: {
              ...
              start: () => {
                const startPin =
                  (window.innerHeight - startThumbsCaption.offsetHeight) / 2;
                return "top +=" + startPin;
              },
              end: () => {
        ...
        if ($container.is(":hover")) {
          player.play().catch((err) => {
            if (err.name === "PlayInterrupted") {
              console.log(
                "Vimeo play interrupted – likely due to rapid hover toggle. Ignored.",
              );
            } else {
  ...
  (function () {
    ...
    function changeSlide(isRight, $controlEl) {
      ...
      var $curActive = document.querySelector(
      ...
      var $newActive = document.querySelector(
      ...
      var currentHeading = $curActive.querySelector(
        ".tp-portfolio-revealing-slide-heading",
      );
      var nextHeading = $newActive.querySelector(
        ".tp-portfolio-revealing-slide-heading",
      );
  ...
  if ($(".cnt-portfolio-ptb").length) {
    ...
    mm.add("(min-width: 992px)", () => {
      ...
      // For px-step-item
      document
        .querySelectorAll(".cnt-portfolio-video-wrapper")
        .forEach((item, i) => {
          gsap.to(item, {
            scrollTrigger: {
              trigger: item,
      ...
      document
        .querySelectorAll(".cnt-portfolio-video-card")
        .forEach((card, i) => {
  ...
  ////////////////////////////////////////////////////
  // Team Page Navigation Control - Dynamic toggle based on admin settings
  if (document.readyState === "loading") {
    document.addEventListener("DOMContentLoaded", initTeamNavigation);
  } else {
    initTeamNavigation();
  }
  ...
  function initTeamNavigation() {
    ...
    fetch(apiPath)
      ...
      .then((data) => {
        ...
        if (!data.enabled) {
          // Team module is disabled - hide Team links
          ...
          // Team module is enabled - show Team links
          teamLinks.forEach((link) => {
            const listItem = link.closest("li");
            if (listItem) {
  ...
  $(function () {
    ...
    const logoSrc =
      $(".tp-header-logo img").first().attr("src") ||
    ...
    const normalizedLogoSrc =
      logoSrc.startsWith("http") || logoSrc.startsWith("/")
        ? logoSrc
        : relativePrefix + logoSrc.replace(/^\.\.\//, "");
    ...
    if (desktopMenuUl.length > 0) {
      ...
      $(
        ".custom-mobile-menu-list .tp-submenu, .custom-mobile-menu-list .submenu",
      ).each(function () {
        $(this)
          .removeClass("tp-submenu submenu")
          .addClass("custom-mobile-drawer-submenu")
          .hide();
        const parentLink = $(this).siblings("a").first();
        if (parentLink.find(".custom-mobile-menu-arrow").length === 0) {
          parentLink.append(
            ' <i class="fa-light fa-chevron-down custom-mobile-menu-arrow"></i>',
          );
        }
      });
    ...
    $(document).on(
      "click",
      ".custom-mobile-drawer-close, .custom-mobile-overlay",
      function () {
        closeDrawer();
      },
    );
    ...
    $(document).on("click", ".custom-mobile-menu-list > li > a", function (e) {
      ...
      if (submenu.length > 0) {
        ...
        if (parentLi.hasClass("active")) {
          ...
          $(".custom-mobile-menu-list .custom-mobile-menu-arrow").css(
            "transform",
            "",
          );
    ...
    $(document).on(
      "click",
      '.custom-mobile-drawer-submenu a, .custom-mobile-menu-list > li > a[href]:not([href="#"])',
      function () {
...
(function () {
  ...
  fetch(apiEndpoint)
    ...
    .catch(function (err) {
      console.warn("Maintenance bypass:", err);
    });
})();
