...
(function ($) {
  ...
  // 24. panel pin section
	...
	(function(){
		const sections = Array.from(document.querySelectorAll('.tp-hero-cst-bottom'));
		...
		sections.forEach(section => {
			...
			const btnItems = Array.from(section.querySelectorAll('.tp-hero-cst-bottom-btn ul li.service__item-8'));
			...
			btnItems.forEach((el, idx) => {
				el.addEventListener('click', () => { current = idx; resetAutoplay(); });
				el.addEventListener('mouseenter', () => { current = idx; resetAutoplay(); });
			});
			...
			function shouldIgnoreEvent(e){
				if(e.target.closest('a, button, input, textarea, select, label')) return true;
			...
			function onUp(e){
				if(!isDown) return;
				isDown = false;
				if(bgContainer) gsap.to(bgContainer, {x: 0, duration: 0.3, ease: 'power2.out'});
				const x = getClientX(e);
				const dx = x - startX;
			...
			// Mouse fallbacks
			document.addEventListener('mousedown', function(e){ if([sliderWrapper, bgContainer].some(t=> t.contains(e.target))) onDown(e); });
			document.addEventListener('mousemove', onMove);
			document.addEventListener('mouseup', onUp);

			// keyboard navigation
			bgContainer.setAttribute('tabindex', '0');
			[sliderWrapper, bgContainer].forEach(t => t.setAttribute && t.setAttribute('tabindex', '0'));
			[sliderWrapper, bgContainer].forEach((t) => t.addEventListener('keydown', (e)=>{
